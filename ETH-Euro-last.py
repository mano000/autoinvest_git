...
while True:
    ...
    #miramos el numero de ordenes abiertas.
    resultado= get_order_list()
    for x in resultado:
    ...
    if (ordenes_abiertas == 0):
        cancela_todo()
        ejecutado=0  #Se pone a cero despues de que no haya ordenes abiertass.
        time.sleep (1)
        ...
        target_price_sell = round (((last_buy_price_media+2)*multiplicador),2)
        target_price_buy  = round (((last_sell_price_media-2)*(1/multiplicador)),2)

        if ((target_price_sell ) >= (actual_price ) ): #+spread
            #sellresult = vende(target_price_sell,0.01*indicador_ventas)   #Si llego aqui se vende
            sellresult = vende(target_price_sell,volumen)
    ...
    if ((ordenes_abiertas == 1) and (ejecutado==0)):
        ...
        for x in resultado:
            for y in x:
                ...
                if ((t["status"] == 'open') and (t["side"] == 'sell')):
                    ...
                    #En algun punto hay que resetear, cuando se haya ejecutado la venta tenemos que poner a cero el precio objetivo...
                    # probablemente es buena idea hacerlo en los dos lados tanto en compras como en ventas... En cuanto la ganancia de las ultimas operaciones sea positiva...
                    # volviendo a poner las ganancias a cero.
                        #also así como:
                        #
                        # if (numero de compras > numero ventas) and (ganancia>1)
                        #   last_sell_price_media=actual_pricex
                        #
                        #
                    ejecutado=1
